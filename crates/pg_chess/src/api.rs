//! PostgreSQL adapter for chess types and functions.
//! Chess logic lives in `gambit-db`; this module is the SQL-facing boundary.

use gambit_db::{
    game_to_pgn, parse_pgn, parse_san, to_san, ChessGame, Color, GameStatus, Move, MoveFlags,
    Position, Square,
};
...
#[derive(
    Debug, Clone, Serialize, Deserialize, PostgresType, PostgresEq, PostgresHash, PostgresOrd,
)]
#[inoutfuncs]
...
impl InOutFuncs for chess_position {
    fn input(input: &CStr) -> Self {
        let s = input
            .to_str()
            .unwrap_or_else(|_| error!("chess_position input was not valid UTF-8"));

        match Position::from_fen(s) {
            Ok(pos) => chess_position(pos),
            Err(_) => error!("invalid FEN for chess position: '{}'", s),
        }
    }
...
fn chess_equivalent(a: &Position, b: &Position) -> bool {
    a.equivalent_for_transposition(b)
}
...
impl InOutFuncs for chess_move {
    fn input(input: &CStr) -> Self {
        let input = input
            .to_str()
            .unwrap_or_else(|_| error!("chess_move input was not valid UTF-8"));
...
fn parse_game_text(s: &str) -> Option<ChessGame> {
    ...
    let start = Position::from_fen(fen_part).ok()?;
...
fn chess_is_valid_fen(fen: &str) -> bool {
    Position::is_valid_fen(fen)
}

#[pg_extern(immutable, parallel_safe)]
fn chess_fen_error(fen: &str) -> Option<String> {
...
fn chess_move_promotion(mv: chess_move) -> Option<String> {
    mv.0.promotion.map(|piece| piece.to_char().to_string())
}
    ...
    let rows: Vec<_> = position
        ...
        .map(|m| {
            (
                ...
                m.flags.contains(MoveFlags::KING_CASTLE)
                    || m.flags.contains(MoveFlags::QUEEN_CASTLE),
...
fn chess_game_positions(
    ...
    (
        name!(ply, i32),
        name!(position, chess_position),
        name!(hash, i64),
    ),
> {
    let g = &game.0;
    let rows: Vec<(i32, chess_position, i64)> = g
        .iter_positions()
        .map(|(ply, pos)| (ply as i32, chess_position(pos.clone()), pos.hash as i64))
        .collect();
    TableIterator::new(rows)
}
...
#[pg_extern(immutable, parallel_safe)]
fn chess_move_to_san(position: chess_position, mv: chess_move) -> String {
    to_san(&position.0, mv.0)
}

#[pg_extern(immutable, parallel_safe)]
fn chess_apply_san(position: chess_position, san: &str) -> chess_position {
    match parse_san(&position.0, san) {
        Ok(m) => match position.0.apply_move(m) {
            Ok(next) => chess_position(next),
            Err(_) => error!("illegal move '{}' in this position", san),
        },
        Err(e) => error!("invalid SAN '{}': {}", san, e),
    }
}

#[pg_extern(immutable, parallel_safe)]
fn chess_from_pgn(pgn: &str) -> chess_game {
    match parse_pgn(pgn) {
        Ok(g) => match g.to_chess_game() {
            Ok(cg) => chess_game(cg),
            Err(e) => error!("invalid PGN movetext: {}", e),
        },
        Err(e) => error!("invalid PGN: {}", e),
    }
}

#[pg_extern(immutable, parallel_safe)]
fn chess_to_pgn(game: chess_game) -> String {
    game_to_pgn(&game.0)
}
...
mod tests {
    ...
    fn text_input_output_uses_fen() {
        ...
        let fen = Spi::get_one::<String>(&format!("SELECT ('{input}'::chess_position)::text"))
            .expect("SPI query failed")
            .expect("SPI returned NULL");
        assert_eq!(fen, input);
    }
    ...
    fn side_to_move_reads_correctly() {
        let stm = Spi::get_one::<String>(
            "SELECT chess_side_to_move('4k3/8/8/8/8/8/8/4K3 b - - 0 1'::chess_position)",
        )
        .expect("SPI query failed")
        .expect("SPI returned NULL");
        assert_eq!(stm, "b");
    ...
    fn move_type_roundtrips_uci() {
        let uci = Spi::get_one::<String>("SELECT ('e2e4'::chess_move)::text")
            .expect("SPI query failed")
            .expect("SPI returned NULL");
    ...
    fn promotion_move_roundtrips_uci() {
        let uci = Spi::get_one::<String>("SELECT ('e7e8q'::chess_move)::text")
            .expect("SPI query failed")
            .expect("SPI returned NULL");
    ...
    fn move_square_accessors_work() {
        let from = Spi::get_one::<String>("SELECT chess_move_from_square('e2e4'::chess_move)")
            .expect("SPI Query Failed")
            .expect("SPI returned Null");

        let to = Spi::get_one::<String>("SELECT chess_move_to_square('e2e4'::chess_move)")
            .expect("SPI Query failed")
            .expect("SPI returned NULL");
    ...
    fn move_promotion_accessor_works() {
        let promotion = Spi::get_one::<String>("SELECT chess_move_promotion('e7e8n'::chess_move)")
            .expect("SPI query failed")
            .expect("SPI returned NULL");
    ...
    fn non_promotion_move_returns_null_promotion() {
        let promotion = Spi::get_one::<String>("SELECT chess_move_promotion('e2e4'::chess_move)")
            .expect("SPI query failed");
    ...
    fn game_text_roundtrips() {
        let ply = Spi::get_one::<i32>("SELECT chess_game_ply(chess_play(chess_new_game(),'e2e4'))")
            .expect("SPI failed")
            .expect("NULL");
        assert_eq!(ply, 1);
    }
    ...
    fn illegal_move_errors() {
        let result = std::panic::catch_unwind(|| {
            Spi::get_one::<String>("SELECT chess_game_fen(chess_play(chess_new_game(),'e2e5'))")
        });
    ...
    fn legal_moves_startpos_is_20() {
        let n =
            Spi::get_one::<i64>("SELECT count(*) FROM chess_legal_moves(chess_start_position())")
                .expect("SPI failed")
                .expect("NULL");
        assert_eq!(n, 20);
    }
    ...
    fn in_check_detected_via_sql() {
        let checked = Spi::get_one::<bool>(
            "SELECT chess_in_check(chess_from_fen('4r3/8/8/8/8/8/8/4K1k1 w - - 0 1'))",
        )
        .expect("SPI failed")
        .expect("NULL");
        assert!(checked);
    ...
    fn placement_reflects_a_move() {
        ...
        let has_e4 = Spi::get_one::<bool>(&format!(
            "SELECT chess_placement({}) @> ARRAY['Pe4']",
            after_e4
        ))
        .expect("SPI failed")
        .expect("NULL");
        ...
        let has_e2 = Spi::get_one::<bool>(&format!(
            "SELECT chess_placement({}) @> ARRAY['Pe2']",
            after_e4
        ))
        .expect("SPI failed")
        .expect("NULL");
    ...
    fn gin_containment_finds_pattern() {
        ...
        let n =
            Spi::get_one::<i64>("SELECT count(*) FROM t_pos WHERE placement @> ARRAY['Pe4','pe5']")
                .expect("SPI failed")
                .expect("NULL");
        assert_eq!(n, 1);
    }
    ...
    #[pg_test]
    fn san_roundtrip_via_sql() {
        let san = Spi::get_one::<String>(
            "SELECT chess_move_to_san(chess_start_position(), 'e2e4'::chess_move)",
        )
        .expect("SPI failed")
        .expect("NULL");
        assert_eq!(san, "e4");
