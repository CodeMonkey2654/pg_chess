...
/// Whether a paginated result has another page after the current window.
pub fn page_has_more(offset: i64, limit: i64, total: i64) -> bool {
    offset + limit < total
}
...
mod tests {
    ...
    fn download_progress_with_total() {
        let msg = format_download_progress("2024-01", 2 * 1024 * 1024 * 1024, Some(30 * 1024 * 1024 * 1024));
    ...
    #[test]
    fn page_has_more_edges() {
        assert!(!page_has_more(0, 20, 15));
        assert!(page_has_more(0, 20, 21));
        assert!(!page_has_more(20, 20, 40));
    }
}
